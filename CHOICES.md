...
This document explains every major technology choice in StoreMind AI and why I picked it over alternatives.
...
**Re-ID Strategy:**
For intra-camera tracking (within the same video), I use **distance-based trajectory** via ByteTrack. 
To satisfy the rubric's `REENTRY` requirement (where a person leaves and comes back), I would use an **OSNet / torchreid** appearance embedding model, invoked *only* when an `ENTRY` occurs, comparing the new visitor's embedding against a gallery of recent `EXIT` embeddings. This hybrid approach (ByteTrack for continuous tracking + OSNet only at the door) keeps the pipeline blazing fast.
...
**Decision Rationale:**
Supabase gives me a powerful PostgreSQL database with **built-in Realtime** subscriptions — essential for my <3 second dashboard update target. The free tier is generous enough for hackathon scale, and I get Auth + Row Level Security for free. Most importantly, SQL lets me do complex analytics queries (GROUP BY zone, time-series) that would require external aggregation with NoSQL solutions.
...
**Decision Rationale:**
Qdrant's **free 1GB cloud tier** is perfect for my use case. I store ~100 analytics documents per day — vastly within limits. Qdrant's **payload filtering** allows me to retrieve only metrics from the last 7 days, giving the Copilot time-aware context. The Python client is well-documented and integrates cleanly with my FastAPI backend.
...
**Decision Rationale:**
BGE-small outperforms OpenAI Ada v2 on the MTEB benchmark while being completely **free and local**. The 384-dimensional vectors are small enough for fast retrieval without sacrificing quality. The BGE models specifically benefit from instruction-tuned retrieval prompts (`"Represent this sentence for searching relevant passages: "`), which I use in my pipeline.
...
**What I Chose and Why:** I **overrode** the AI's suggestion and chose a **Flat, Wide NoSQL-style Schema** mapped to a single PostgreSQL `events` table with a JSONB `metadata` column.
*Why?* The AI's normalized approach would require 3-4 separate INSERT statements (or complex UPSERTs) for every single video frame event, crippling database write throughput at 30 FPS. By keeping it flat, I can batch-insert events effortlessly. I can still achieve fast analytics by adding standard B-Tree indices on `event_type` and `visitor_id`, while keeping dynamic properties (like `is_staff` or `dwell_ms`) flexibly packed inside the JSONB metadata column.
